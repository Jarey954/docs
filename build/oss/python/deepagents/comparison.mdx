...
| **Deployment** | [`deepagents deploy`](/oss/python/deepagents/deploy) | Self-hosted | N/A |
...
- **Choose Deep Agents** to go to production with one command: a model-agnostic, [multi-tenant](/oss/python/deepagents/going-to-production#multi-tenancy) deployment with built-in auth, observability, and open protocol endpoints, without assembling your own hosting stack, while [keeping control of your memory](https://www.langchain.com/blog/your-harness-your-memory).
...
| ------- | --------------- | -------------------- | ------------- |
| Production hosting | ✅ [LangSmith Deployment](/oss/python/langgraph/deploy), [`deepagents deploy`](/oss/python/deepagents/deploy), self-hosted, or standalone | Build your own HTTP/WebSocket layer; run SDK in [containers](https://platform.anthropic.com/docs/en/agent-sdk/hosting) | ❌ |
| Multi-tenant deployment | ✅ One deployment with [custom auth and scoped user data](/oss/python/deepagents/going-to-production#multi-tenancy), plus [RBAC](/langsmith/rbac) | ⚠️ Possible, but you must build auth, tenancy, and per-user isolation yourself | ❌ |
| Local execution environment | ✅ | ✅ | ✅ |
| Cloud execution environment | ❌ | ❌ | ✅ |

<Info>
    Need user-level isolation in one shared deployment? Deep Agents support [multi-tenant deployments](/oss/python/deepagents/going-to-production#multi-tenancy) with scoped threads, runs, and conversation history. For a walkthrough, watch the [custom auth video](https://www.youtube.com/watch?v=DkNqgCz8cjE).
</Info>
...
| MCP client (connect external tools) | ✅ | ✅ [MCP](https://platform.claude.com/docs/en/agent-sdk/mcp) | ✅ [MCP client](https://developers.openai.com/codex/mcp) |
...
| Skills | ✅ [Skills](/oss/python/deepagents/skills) | ✅ [Skills](https://platform.claude.com/docs/en/agent-sdk/skills) | ✅ [Skills](https://developers.openai.com/codex/skills) |
| Agent instructions file (loaded at startup) | ✅ [Memory](/oss/python/deepagents/customization#memory) | ✅ [CLAUDE.md files](https://platform.claude.com/docs/en/agent-sdk/modifying-system-prompts#method-1-claude-md-files-project-level-instructions) | ✅ [AGENTS.md](https://developers.openai.com/codex/guides/agents-md) |
| Long-term memory (persist context across sessions) | ✅ [Memory Store](/oss/python/deepagents/memory) | ❌ | ❌ |
...
| Middleware / lifecycle hooks | ✅ | ✅ [Hooks](https://platform.claude.com/docs/en/agent-sdk/hooks) | ❌ |
| Virtual filesystem (pluggable storage backends) | ✅ [Virtual filesystem](/oss/python/deepagents/harness#virtual-filesystem-access) with [pluggable backends](/oss/python/deepagents/backends) | ❌ | ❌ |

### Protocols and state

| Feature | **Deep Agents** | **Claude Agent SDK** | **Codex SDK** |
| ------- | --------------- | -------------------- | ------------- |
| ACP server (expose agent to IDEs) | ✅ [ACP server](/oss/python/deepagents/acp) | ✅ (third-party) | ✅ (third-party) |
| MCP server mode (expose agent as a tool) | ✅ [MCP endpoint through Agent Server](/langsmith/server-mcp) | ❌ | ✅ [`codex mcp-server`](https://developers.openai.com/codex/guides/agents-sdk) |
| A2A endpoint (agent-to-agent protocol) | ✅ [A2A endpoint in Agent Server](https://docs.langchain.com/langsmith/server-a2a#a2a-endpoint-in-agent-server) | ✅ (third-party) | ❌ |
| Session resume (reload conversation history) | ✅ [Reconnect and load thread history](/oss/python/deepagents/going-to-production#frontend) | ✅ [Session management](https://platform.claude.com/docs/en/agent-sdk/sessions) | ✅ [Resume threads](https://developers.openai.com/codex/cli/features#resuming-conversations); SDK `resumeThread()` |
| Checkpoint to disk (resume across restarts) | ✅ [Backends and checkpoints](/oss/python/deepagents/backends#built-in-backends) | ✅ [File checkpointing](https://platform.claude.com/docs/en/agent-sdk/file-checkpointing) | ✅ [Git checkpoints](https://developers.openai.com/codex/config-basic) |
